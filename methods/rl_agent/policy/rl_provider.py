...
(``pcbworld_projection`` → ``env.step``) handles RL output unchanged.
...
def _build_action_envelope(
    ...
    # letting pcbworld_projection emit its _FALLBACK_ACTION (idle).
...
class RLPolicyProvider:
    ...
    def generate(self, prompt_pairs, observations=None, *,
        ...
        ``pcbworld_projection → env.step`` doesn't care.
        """
        import torch
        ...
        for i, obs in enumerate(observations):
            ...
            # pcbworld_projection → env.step path consumes the RL output
