---
date: 2026-05-06
purpose: How to run pcbworld eval.metrics on routed PCBs and aggregate per-method comparison numbers (succ / DRV / WL / via / Φ). Designed for adding NEW methods (e.g. a new LLM) under the existing routed-PCB root, reusing the existing source `.kicad_pro` files.
...
`$PCBWORLD_DATA_ROOT` is the dataset root (layout: `configs/paths.yaml`);
...
$PCBWORLD_DATA_ROOT/synthetic/synth_2L_v2/test/<sample>.kicad_pro
$PCBWORLD_DATA_ROOT/pcbench/exacad_sorted/<sample>/processed_v9_guide_v3.kicad_pro
...
for d in $PCBWORLD_DATA_ROOT/pcbench/exacad_sorted/*/; do
    sample=$(basename "$d")
...
for f in $PCBWORLD_DATA_ROOT/synthetic/synth_2L_v2/test/board_*.kicad_pro; do
...
You may need to point to YOUR pcbworld install. Set these once:

```bash
export PCBWORLD_ROOT=/path/to/your/pcbworld     # this repo's checkout root
export PY=/path/to/pcbworld/conda/env/python    # the conda env that built pcbworld
export EVAL_PY=/path/to/your/scoring/wrapper    # not shipped — see the note above
export EVAL_OUTDIR="$PCBWORLD_ROOT/eval"        # where summaries land

export PYTHONPATH=$PCBWORLD_ROOT/build_rl/pcbnew/python/rl:$PCBWORLD_ROOT
```
...
- **All boards have `error` field** → pcbworld build_rl env vars not set.
