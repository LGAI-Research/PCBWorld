"""Eval script for the pcbworld KiCad PCB routing environment.
...
Usage:
    ...
    python methods/llm_agent/rollout/pcbworld.py --board_path /path/to/board.kicad_pcb

    # LLM rollout (requires GPU)
    python methods/llm_agent/rollout/pcbworld.py --mode llm --model_path Qwen/Qwen2.5-VL-7B-Instruct \
        --rollout_episodes 10 --max_steps 50

    # Large model on specific GPUs
    python methods/llm_agent/rollout/pcbworld.py --mode llm --model_path meta-llama/Llama-3.1-70B-Instruct \
...
def parse_args() -> argparse.Namespace:
    ...
    return build_eval_parser(description="pcbworld environment eval").parse_args()
...
class _EvalRolloutManager(KiCadLLMRolloutManager):
    ...
    OmegaConf shape (``config.env.pcbworld.prompt_version``); eval scripts
    ...
    def __init__(self, envs, args):
        ...
        super().__init__(
            envs, pcbworld_projection, args, board_scheduler=None,
            score_episodes=True,
        )
...
def main() -> None:
    ...
    _print_section("pcbworld environment eval")
