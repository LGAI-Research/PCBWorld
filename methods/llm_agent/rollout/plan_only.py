...
IMPORTANT -- conda env: run this in the default ``pcbworld`` env (torch / pcbnew /
...
and applies the loose template rules of ``configs/drc/pcbworld_drc.yaml``
...
# ``plan_only_responses`` dataset under $PCBWORLD_DATA_ROOT; --responses-root
...
def _replay_one(job: dict) -> dict:
    ...
    try:
        ...
        # to the configs/drc/pcbworld_drc.yaml template.
        env = PCBWorld(
            board_path=job["board_path"],
...
def main():
    ...
    ap.add_argument("--responses-root", type=Path, default=None,
                    help="root holding the apiseq_<split>_zs_<model>/ summary dirs. "
                         ...
                         "$PCBWORLD_DATA_ROOT (configs/paths.yaml).")
