...
``config.env.env_name`` is pcbworld; other env types keep verl's own
``fit()`` loop.
...
module importable in the publish-side pcbworld repo without verl installed.
"""
...
_RL_KEY_FROM_VERL: dict[str, str] = {
    ...
    # rollout/* — episode aggregates (pcbworld 6-metric panel + reward/length)
...
class LLMTrainer(Trainer):
    ...
    def on_iteration_end(self, iteration: int, metrics: dict) -> None:
        ...
        #     trainer.best_metric_key (run_pcbworld*.sh) below, RL via
