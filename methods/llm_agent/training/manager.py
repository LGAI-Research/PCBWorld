"""Trainer-agnostic rollout-manager base for the pcbworld KiCad routing env.

Owns all pcbworld-specific state and per-step logic that is shared across
training algorithm adapters (verl-agent today, future RL trainers tomorrow):
  ...
  - versioned prompt bundle dispatch (``get_pcbworld_prompts``)
...
from methods.llm_agent.wrappers.prompts import get_pcbworld_prompts
...
class KiCadLLMRolloutManager:
    ...
    def __init__(
        ...
        self._prompts = get_pcbworld_prompts(prompt_version)
    ...
    def _resolve_prompt_version(self, config) -> str:
        """Default schema: ``config.env.pcbworld.prompt_version`` (verl-agent).

        Default sourced from the shared schema (configs/pcbworld.yaml (llm_train:))
        ...
        cad_cfg = self._cfg_get(env_cfg, "pcbworld", None)
        ...
        for i, scored in zip(indices, scored_list):
            ...
            if infos is not None and i < len(infos):
                ...
                runtime.update({
                    ...
                    # success (same flag as rollout/pcbworld.py; LLM-owned,
    ...
    def build_text_obs(
        ...
        for i in range(len(text_obs)):
            ...
            system = prompts.PCBWORLD_SYSTEM_PROMPT.format(
            ...
            if init or self._history_length <= 0 or (not history_text and not rejected):
                user = prompts.PCBWORLD_USER_PROMPT_NO_HIS.format(
                    dynamic_observation=dynamic_obs,
                    valid_actions=valid_actions_str,
                )
            else:
                user = prompts.PCBWORLD_USER_PROMPT.format(
