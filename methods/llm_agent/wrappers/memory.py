...
"""Per-environment history container used by the cadagent eval loops.

``SimpleMemory`` is referenced from verl-agent
(``external/verl-agent/agent_system/memory/memory.py``) and inlined here
so the LLM eval path does not need ``external/verl-agent`` on ``sys.path``.
"""
