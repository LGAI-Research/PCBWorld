# --------------------- PCBWorld V5 (KiCad PCB Routing) --------------------- #
#
# V5 = V4 + an ephemeral "last rejected attempt" line injected into the user
# prompt.
#
# Motivation: V4 history only stores valid+success steps, so when the model
# emits the same mask-rejected action repeatedly the prompt shows nothing
# about those rejections — the next turn's context looks identical to the
# previous, and the policy gets stuck in a self-reinforcing loop (e.g. 30
# consecutive `make_line` rejects observed at training step 20). V5 surfaces
# just enough negative signal to break the loop without bloating
# history_length:
#
#   - On every invalid step (parse fail OR action-mask reject), update a
#     per-env rejection streak with the raw action body (NO <think> block).
#   - Only the most recent streak is kept — a new body replaces the previous
#     one. Consecutive identical bodies are collapsed to "(rejected ×N)".
#   - The streak is volatile: any successful valid step clears it.
#   - The streak renders into the new {rejected_attempts} slot. When there is
#     no active streak the slot resolves to "" so the prompt is byte-identical
#     to V4 in the common case.
#
# Interface: same six exported symbols as V1–V4, plus one extra format kwarg
# ({rejected_attempts}) on PCBWORLD_USER_PROMPT and PCBWORLD_TEMPLATE. All
# str.format() callers must pass it (env_manager + the pcbworld rollout updated).
# Older versions ignore the extra kwarg, so passing it is always safe.
# -----------------------------------------------------------------------------

# ======================================================================
# Board state format descriptions (same as V1–V4)
# ======================================================================
...
# which case the prompt is structurally identical to V4. When non-empty, the
