...
#
# Interface-compatible with V1/V2 (drop-in via PCBWORLD_PROMPT_VERSION=v3
# switch in pcbworld.py). All exported names match V1. PCBWORLD_USER_PROMPT
# keeps the wschoi-branch history format `{current_step} / {valid_step_count}`
# to match rollout/pcbworld.py and env_manager.py callers.
# -----------------------------------------------------------------------------
...
- Your targets are the (point ...) entries listed under each net in routing_geometry.
  Each point represents a pad that still needs to be connected.
- Never place a via directly on a pad — offset at least 1mm from the pad first
  before issuing make_via. Via drills on pads create solderability issues.
- Always use exact coordinates from the observation (pad positions in board_static,
  (point ...) targets, or track endpoints in routing_geometry). Only compute
  coordinates for intermediate waypoints when you must detour around an obstacle."""
...
_FEW_SHOT_EXAMPLE_1 = """\
# Few-Shot Example 1: Via Crossing (avoid another net's track)
...
(After net_end NET3 is finalized; router_head returns to phase 0 to choose the
next net. Example ends here.)"""


# ----------------------------------------------------------------------
# Few-Shot Example 2 — Layer-Change Detour (12x12mm, 1 net, static obstacle)
#   NET1: P1(0,0) -> P2(10,10). A static obstacle on layer 1 blocks the
#   diagonal path, so the agent detours through layer 2.
# ----------------------------------------------------------------------

_FEW_SHOT_EXAMPLE_2 = """\
# Few-Shot Example 2: Layer-Change Detour (avoid a static obstacle)

Scenario: a 12x12mm board with 1 net. A static obstacle on layer 1 blocks the
direct diagonal path between P1(0,0) and P2(10,10). The agent goes around it
on layer 2 and comes back up near the target pad.

board_static (different board from Example 1):
(board_static
  (bbox 0.000 0.000 12.000 12.000)
  (scale 12.000)
  (net_count 1)
  (copper_layers 2)
  (nets
    (net 1 "PWR"
      (pads (pad D0 0.000 0.000 1) (pad D1 10.000 10.000 1))))
  (obstacles
    (obs O0 5.000 5.000 (size 3.000 3.000) (layer 1))))
...
(routing_geometry
  (net 1
    (tracks)
    (vias)
    (points (point Q0 0.000 0.000) (point Q1 10.000 10.000))))
(router_head
  (xy 0.000 0.000)
  (layer 1)
  (net 0)
...
Response:
<think>Only one net to route. The obstacle at (5,5) on layer 1 sits right on the diagonal between P1(0,0) and P2(10,10). I will detour through layer 2.</think>
<action>net_select 1</action>
...
Response:
<think>Start routing from P1 at (0.0, 0.0) on layer 1.</think>
<action>start_route 0.0 0.0 1</action>
...
Response:
<think>I am on a pad; I must move off it before placing a via. Nudge 0.5mm downward and drop a via to reach layer 2.</think>
<action>make_via 0.0 0.5 w</action>

--- Step 3 -----------------------------------------------------------------
Observation:
(routing_geometry
  (net 1
    (tracks (track T0 (p1 P0 0.000 0.000) (p2 P1 0.000 0.500) (layer 1)))
    (vias (via V0 (center P1 0.000 0.500) (layers 1 2)))
    (points (point Q1 10.000 10.000))))
(router_head
  (xy 0.000 0.500)
  (layer 2)
...
Response:
<think>On layer 2 now. The obstacle is only on layer 1, so I have a clear diagonal on layer 2. I'll route near P2 but stop 0.5mm short so I can safely place a via without landing on the pad.</think>
<action>make_line 10.0 9.5 w</action>
...
(routing_geometry
  (net 1
    (tracks
      (track T0 (p1 P0 0.000 0.000) (p2 P1 0.000 0.500) (layer 1))
      (track T1 (p1 P1 0.000 0.500) (p2 P2 10.000 9.500) (layer 2)))
    (vias (via V0 (center P1 0.000 0.500) (layers 1 2)))
    (points (point Q1 10.000 10.000))))
(router_head
  (xy 10.000 9.500)
...
Response:
<think>Clear of the obstacle at (10, 9.5). Switch back to layer 1 here, not on P2, to keep the via off the pad.</think>
<action>make_via 10.0 9.5 w</action>
...
(routing_geometry
  (net 1
    (tracks
      ...
      (track T1 (p1 P1 0.000 0.500) (p2 P2 10.000 9.500) (layer 2)))
    (vias
      (via V0 (center P1 0.000 0.500) (layers 1 2))
      (via V1 (center P2 10.000 9.500) (layers 1 2)))
    (points (point Q1 10.000 10.000))))
(router_head
  (xy 10.000 9.500)
  (layer 1)
...
Response:
<think>Back on layer 1, 0.5mm from P2. A short finish will auto-complete to the remaining point.</think>
<action>finish w</action>
...
(routing_geometry
  (net 1
    (tracks
      (track T0 (p1 P0 0.000 0.000) (p2 P1 0.000 0.500) (layer 1))
      (track T1 (p1 P1 0.000 0.500) (p2 P2 10.000 9.500) (layer 2))
      (track T2 (p1 P2 10.000 9.500) (p2 P3 10.000 10.000) (layer 1)))
    (vias
      (via V0 (center P1 0.000 0.500) (layers 1 2))
      (via V1 (center P2 10.000 9.500) (layers 1 2)))
    (points)))
...
Response:
<think>All points are gone; NET1 is fully connected. Finalize.</think>
<action>net_end</action>

(Example ends. Both examples share the same pattern: identify blocked layer,
detour via layer switch, return for the final segment.)"""

_FEW_SHOT_EXAMPLES = f"{_FEW_SHOT_EXAMPLE_1}\n\n{_FEW_SHOT_EXAMPLE_2}"
...
# kwargs in rollout/pcbworld.py and agent_system/environments/env_manager.py.
PCBWORLD_USER_PROMPT = """\
## Step {current_step}
{dynamic_observation}
