"""Pluggable LLM/API providers for pcbworld evaluation.

Encapsulates the two action-source backends that ``rollout/pcbworld.py`` (batch
...
def _call_together(system_prompt, user_prompt, model, temperature, max_tokens,
    ...
    text = msg.content or ""
    # pcbworld_projection requires a well-formed (non-empty) ``<think>...</think>``
        ...
        for system_prompt, user_prompt in prompt_pairs:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    # No ``<action>`` tag → pcbworld_projection returns its
