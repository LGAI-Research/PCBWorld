...
# field defaults from here. Edit configs/pcbworld.yaml (one section per surface) to change a default.
_CONFIGS_DIR = Path(__file__).resolve().parent.parent
_DEFAULTS = load_config(_CONFIGS_DIR / "pcbworld.yaml")
# Values the paper-era checkpoints were trained with for knobs that did not exist yet: a
# checkpoint whose stored args lack such a key is read as if it carried these (they live
# with the paper recipes, not in code — see with_legacy_ckpt_defaults / tools/ckpt_migrate.py).
_LEGACY_CKPT_PATH = _CONFIGS_DIR.parent / "experiments" / "kdd" / "configs" / "legacy_ckpt_defaults.yaml"
_LEGACY_CKPT = load_config(_LEGACY_CKPT_PATH)


def _none_if_off(value: Any) -> Any:
    """CLI spelling for "no directional candidates": ``none`` / ``off`` / empty -> None."""
    if isinstance(value, str) and value.strip().lower() in ("none", "off", ""):
        return None
    return value


def with_legacy_ckpt_defaults(ckpt_args: dict[str, Any]) -> dict[str, Any]:
    """Return a copy of a checkpoint's stored ``args`` with every key of
    experiments/kdd/configs/legacy_ckpt_defaults.yaml filled in when absent — the value the
    checkpoint was trained with before that knob existed. New checkpoints store every knob,
    so this only ever touches pre-knob (paper-era) files; ``tools/ckpt_migrate.py`` writes the
    same values into the file for good."""
    out = dict(ckpt_args)
    for section in _LEGACY_CKPT.values():
        for k, v in section.items():
            out.setdefault(k, v)
    return out
_ENV = _DEFAULTS["env"]
_REWARD = _ENV["reward"]
_POLICY = _DEFAULTS["rl_policy"]
...
class EnvConfig:
    ...
    drc_config_path: str | None = field(default=_ENV["drc_config_path"], metadata={
        "help": "YAML DRC config used together with --use-yaml-drc-fallback. "
                ...
                "(e.g. configs/drc/pcbworld_drc.yaml)."})
...
class RLEnvConfig:
    ...
    directional_candidates: str | None = field(default=_ENV["directional_candidates"], metadata={
        "help": "'none' turns the ring off. Directional candidate generation mode: a preset name "
    ...
    offboard_mask: bool = field(default=_ENV["offboard_mask"], metadata={
    ...
    def from_namespace(cls, args: Any) -> "RLEnvConfig":
        ...
        return cls(
            ...
            directional_candidates=_none_if_off(getattr(args, "directional_candidates", None)),
    ...
    def from_checkpoint(cls, ckpt_args: dict[str, Any], max_steps: int) -> "RLEnvConfig":
        ...
        configs/pcbworld.yaml (env:)) when the checkpoint lacks it, so an old
        checkpoint evals with the same defaults training would have used.
        """
        _warn_renamed_ckpt_keys(ckpt_args)
        ckpt_args = with_legacy_ckpt_defaults(ckpt_args)   # pre-knob keys -> the paper-era values
        ...
        env = EnvConfig(
            ...
            # Obs-shaping knobs: the stored value, or (pre-knob checkpoint) the paper-era value
            # from legacy_ckpt_defaults.yaml — never the current YAML default, which would
            # silently change how an old checkpoint evals.
            simplify_outline=bool(ckpt_args["simplify_outline"]),
            outline_obs=str(ckpt_args["outline_obs"]),
            ...
            net_constraint_obs=bool(ckpt_args["net_constraint_obs"]),
        ...
        return cls(
            ...
            directional_candidates=(
                _none_if_off(ckpt_args.get("directional_candidates"))
                or (f"grid{int(g)}"
                    if (g := ckpt_args.get("directional_grid_size")) is not None
                    else None)
            ),
            # Candidate-set knobs (they define the pointer index space): stored value or the
            # paper-era value (legacy_ckpt_defaults.yaml) — never the current default.
            connectivity_filter=bool(ckpt_args["connectivity_filter"]),
            pad_graze_margin_mm=float(ckpt_args["pad_graze_margin_mm"]),
            offboard_mask=bool(ckpt_args["offboard_mask"]),
        )
...
class RLPolicyConfig:
    ...
    def from_checkpoint(cls, args: dict[str, Any]) -> "RLPolicyConfig":
        ...
        configs/pcbworld.yaml (rl_policy:)) when the checkpoint lacks it.
        """
        args = with_legacy_ckpt_defaults(args)   # pre-knob keys -> the paper-era values
        ...
        return cls(
            ...
            # checkpoints load.
            n_max_slots=int(args["n_max_slots"]),
            ...
            time_feature=str(args["time_feature"]),   # stored, or the paper-era value (legacy file)
            ...
            obstacle_obs=bool(args["obstacle_obs"]),
...
class PPOConfig:
    ...
    # --- BC-initialised fine-tuning (methods/rl_agent/training/finetune.py, loop.py) ---
    init_ckpt: str | None = field(default=_TRAIN["ppo"]["init_ckpt"], metadata={
        "help": "Initialise the policy WEIGHTS from this checkpoint (e.g. a BC "
                "policy_step_*.pt); optimizer, iteration counter and reward "
                "normalizer start fresh (unlike --resume). A missing critic_head "
                "is newly initialised; obs-affecting args are cross-checked."})
    finetune_lr: float = field(default=_TRAIN["ppo"]["finetune_lr"], metadata={
        "help": "Learning rate used when --init-ckpt is given and --lr is not (fine-tune preset)."})
    finetune_clip_eps: float = field(default=_TRAIN["ppo"]["finetune_clip_eps"], metadata={
        "help": "PPO clip range used when --init-ckpt is given and --clip-eps is not (fine-tune preset)."})
    critic_warmup_iters: int = field(default=_TRAIN["ppo"]["critic_warmup_iters"], metadata={
        "help": "Iterations 1..N train ONLY the critic head: actor + backbone "
                "frozen (detach_critic forced on, policy/entropy/KL terms "
                "dropped) at --critic-warmup-lr; the actor's --warmup-iters LR "
                "ramp starts after them."})
    critic_warmup_lr: float = field(default=_TRAIN["ppo"]["critic_warmup_lr"], metadata={
        "help": "Learning rate during --critic-warmup-iters"})
    critic_warmup_min_ev: float = field(default=_TRAIN["ppo"]["critic_warmup_min_ev"], metadata={
        "help": "EV gate: after --critic-warmup-iters keep the critic-only phase until "
                "train/explained_variance >= this for --critic-warmup-ev-streak consecutive "
                "iterations (0 = fixed-length warm-up). Scratch runs here reach 0.9 by ~iter 20."})
    critic_warmup_ev_streak: int = field(default=_TRAIN["ppo"]["critic_warmup_ev_streak"], metadata={
        "help": "Consecutive iterations the EV gate must hold"})
    critic_warmup_max_iters: int = field(default=_TRAIN["ppo"]["critic_warmup_max_iters"], metadata={
        "help": "Hard cap on the EV-gated warm-up (0 = 3 x --critic-warmup-iters); the actor "
                "phase starts here even if the gate never passed (logged loudly)"})
    kl_ref_coef: float = field(default=_TRAIN["ppo"]["kl_ref_coef"], metadata={
        "help": "beta of the KL(pi || pi_ref) penalty toward a frozen reference "
                "policy (k3 estimator on the rollout actions); 0 = off. Not "
                "supported with --update-gpus > 1."})
    kl_ref_ckpt: str | None = field(default=_TRAIN["ppo"]["kl_ref_ckpt"], metadata={
        "help": "Reference policy checkpoint for --kl-ref-coef (default: --init-ckpt)"})
    kl_ref_seen_drc_types: str = field(default=_TRAIN["ppo"]["kl_ref_seen_drc_types"], metadata={
        "help": "Comma list of DRC token type_ids the reference policy was trained "
                "on (pcb_world.engine.drc taxonomy: 2=unconnected 4=via 6=other are "
                "the connectivity-only tokens DRC-clean BC demos carry). With "
                "--kl-ref-mask-unseen-drc, a sample whose observation holds any "
                "other type_id gets KL weight 0."})
    kl_ref_mask_unseen_drc: bool = field(default=_TRAIN["ppo"]["kl_ref_mask_unseen_drc"],
                                         metadata={"cli_skip": True})
    normalize_adv: bool = field(default=_TRAIN["ppo"]["normalize_adv"], metadata={
        "help": "Per-minibatch advantage normalization (SB3 default)"})
    truncation_bootstrap: bool = field(default=_TRAIN["ppo"]["truncation_bootstrap"], metadata={
        "help": "V(s_next) bootstrap at truncation boundaries (off: treat truncation as termination with V=0)"})
    norm_reward: bool = field(default=_TRAIN["ppo"]["norm_reward"], metadata={
        "help": "SB3-style discounted-return reward normalization"})
...
class RLEvalConfig:
    ...
    # AND LLM (verl trainer.best_metric_key in run_pcbworld*.sh) both select the
...
class LLMEnvConfig:
    ...
    masking_rule: str = field(default=_LLM["env"]["masking_rule"], metadata={
        "help": "Masking rule name (configs/masking/, then experiments/kdd/configs/"
                "masking/) or a YAML path. LLM runs used strict|relaxed."})
    reward_rule: str = field(default=_LLM["env"]["reward_rule"], metadata={
        "help": "Reward rule name (configs/reward/, then experiments/kdd/configs/"
                "reward/) or a YAML path. LLM runs used default|shaped|grpo_final."})
    ...
    drc_config_path: str | None = field(default=_LLM["env"]["drc_config_path"], metadata={
        "help": "YAML path supplying the global minima used by "
                ...
                "there is no implicit default (e.g. configs/drc/pcbworld_drc.yaml)."})
...
class LLMPromptConfig:
    ...
    ``prompt_version``'s choices are dynamic (``PCBWORLD_PROMPT_VERSIONS``), so the
...
class LLMVLLMConfig:
    ...
    guided_grammar: str | None = field(default=_LLM["vllm"]["guided_grammar"], metadata={
        "help": "Name of a grammar registered in methods.llm_agent.wrappers.grammar GRAMMARS "
                "(e.g. 'pcbworld_v1'). When set, generation is constrained via vLLM "
...
# ============================================================================
#
# The pcbworld-specific slice of the verl Hydra config (the ``env.pcbworld.*``
# block + the ``env.*`` knobs pcbworld reads). Distinct from LLMConfig, which is
...
# this schema is the single defaults source for the ``config.env.pcbworld.*``
# fallbacks read by the verl-agent patch + methods/llm_agent/training/manager.py. Flat, to
# mirror the flat ``env.pcbworld`` namespace.

_LLM_TRAIN = _DEFAULTS["llm_train"]


@dataclass(frozen=True)
class LLMTrainConfig:
    """pcbworld LLM(verl)-training config — the ``env.pcbworld.*`` slice.
    ...
    ``build_pcbworld_envs``). Defaults = configs/pcbworld.yaml (llm_train:).
    """
    ...
    # SAME ruler (W5 unification). Override per-run via env.pcbworld.eval_*.
    eval_reward_config: str = DEFAULTS.reward_config
    eval_check_angle: int = DEFAULTS.check_angle
    # misc
    seed: int = _LLM_TRAIN["seed"]
    # required runtime input (env.pcbworld.board_path); no meaningful default
    ...
    def to_env_kwargs(self) -> dict[str, Any]:
        """Flat env-build kwargs for ``build_pcbworld_envs(env_kwargs=...)``.
    ...
    def from_verl_config(cls, config: Any) -> "LLMTrainConfig":
        ...
        env-core / board / prompt / scoring knobs read from ``env.pcbworld.*``;
        ``max_steps`` / ``history_length`` / ``seed`` read from the general
        ``env.*`` block (where pcbworld consumes them). Any key the config omits
        falls back to the shared YAML default (``cls()`` =
        configs/pcbworld.yaml (llm_train:)).
        """
        d = cls()  # shared YAML defaults
        g = cls._cfg_get
        env = g(config, "env", None)
        cad = g(env, "pcbworld", None)
        return cls(
