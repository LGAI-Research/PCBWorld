...
Paths come from configs/paths.yaml; every root is overridable via env (PCBWORLD_*).
The datasets are not distributed with the repo: the data root is $PCBWORLD_DATA_ROOT
...
def expand_data_path(value: str, cfg: Paths | None = None) -> str:
    ...
    location as ``${PCBWORLD_DATA_ROOT}/<sub>`` rather than a machine-specific
    ...
    if missing:
        raise KeyError(
            ...
            f"{', '.join(missing)}. Point PCBWORLD_DATA_ROOT at your dataset "
...
def load_paths(path: str | Path = _PATHS_YAML) -> Paths:
    ...
    roots = raw.get("roots", {})
    # Data root has no baked-in default: unset $PCBWORLD_DATA_ROOT expands to "" → None,
...
def _require_data_root(cfg: Paths, name: str) -> Path:
    if cfg.data is None:
        raise RuntimeError(
            f"dataset '{name}' has no staged copy and PCBWORLD_DATA_ROOT is "
    ...
    def _cli(argv: list[str]) -> int:
        ...
        try:
            if argv[:1] == ["data-root"] and len(argv) == 1:
                ...
                if cfg.data is None:
                    raise RuntimeError(
                        "PCBWORLD_DATA_ROOT is not set and configs/paths.yaml "
                        "carries no default — export PCBWORLD_DATA_ROOT "
