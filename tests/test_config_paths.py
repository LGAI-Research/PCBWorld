...
Pins the single-default-line contract:
  1. $PCBWORLD_DATA_ROOT set   -> it wins (over the paths.yaml default);
...
campaign: kdd
roots:
  data:   ${{PCBWORLD_DATA_ROOT:-{default}}}
  staged: {staged}
  out:    ./var/results
...
def test_env_var_wins_over_yaml_default(tmp_path, monkeypatch):
    monkeypatch.setenv("PCBWORLD_DATA_ROOT", "/tmp/env_root")
...
def test_both_empty_fails_loudly(tmp_path, monkeypatch, scenario):
    if scenario == "yaml_default_empty":
        ...
        monkeypatch.setenv("PCBWORLD_DATA_ROOT", "")
        cfg = paths_mod.load_paths(_write_yaml(tmp_path, "/tmp/yaml_default_root"))
    assert cfg.data is None
    with pytest.raises(RuntimeError) as exc:
        paths_mod.resolve_dataset("demo", cfg)
    assert str(exc.value) == (
        "dataset 'demo' has no staged copy and PCBWORLD_DATA_ROOT is "
        "not set — export PCBWORLD_DATA_ROOT pointing at your dataset "
...
def test_expand_env_var_wins(monkeypatch):
    monkeypatch.setenv("PCBWORLD_DATA_ROOT", "/tmp/env_root")
    assert (
        paths_mod.expand_data_path("${PCBWORLD_DATA_ROOT}/x/y.kicad_pcb")
...
def test_expand_unset_var_falls_back_to_yaml_default(tmp_path, monkeypatch):
    ...
    cfg = paths_mod.load_paths(_write_yaml(tmp_path, "/tmp/yaml_default_root"))
    assert (
        paths_mod.expand_data_path("${PCBWORLD_DATA_ROOT}/x.kicad_pcb", cfg)
...
def test_expand_both_empty_fails_loudly(tmp_path, monkeypatch):
    ...
    assert str(exc.value.args[0]) == (
        "dataset path '${PCBWORLD_DATA_ROOT}/x.kicad_pcb' references unset "
        "environment variable(s): PCBWORLD_DATA_ROOT. Point "
        "PCBWORLD_DATA_ROOT at your dataset root (expected layout: "
        "configs/paths.yaml)."
    )
...
def test_real_paths_yaml_has_no_baked_in_data_root(monkeypatch, tmp_path):
    ...
    assert cfg.data is None
    with pytest.raises(RuntimeError, match="PCBWORLD_DATA_ROOT"):
        paths_mod.resolve_dataset("synth_2L_v2", cfg)
    monkeypatch.setenv("PCBWORLD_DATA_ROOT", str(tmp_path))
    cfg = paths_mod.load_paths()
