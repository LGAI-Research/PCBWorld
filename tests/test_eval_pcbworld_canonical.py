"""The LLM eval (methods/llm_agent/rollout/pcbworld.py) folds into the single shared metric vocabulary.
...
def _llm_result(*, won, reward, wl, via, tracks, rout, fp, sys_tok, resp_tok):
    """Shape of one entry produced by the pcbworld rollout's _run_one_episode."""
    return {
...
def test_result_maps_to_canonical_row():
    ...
    from methods.llm_agent.rollout.pcbworld import _result_to_canonical_row
...
def test_crashed_result_maps_to_status_crashed():
    from methods.llm_agent.rollout.pcbworld import _crashed_result, _result_to_canonical_row
