...
class TestDirectionalModes:
    ...
    def test_mres13_ladder(self):
        # "1-3 dyadic" ladder for the 0.4 mm generation grid: {0.4 * 2^k} u
        # {1.2 * 2^k} + the 0.2 mm fine step; 8 dirs x 13 rungs = 104 candidates.
        ladder = DIRECTIONAL_DISTANCE_PRESETS["mres13"]
        assert ladder == (0.2, 0.4, 0.8, 1.2, 1.6, 2.4, 3.2, 4.8, 6.4, 9.6, 12.8, 25.6, 51.2)
        assert set(ladder) == {0.2} | {0.4 * 2 ** k for k in range(8)} | {1.2 * 2 ** k for k in range(4)}
        assert parse_directional_mode("mres13") == (None, ladder)
        cands = build_directional_candidates((50.0, 50.0), current_layer=1, mode="mres13")
        assert len(cands) == 8 * 13
        for i, (x, y, layer, ctype) in enumerate(cands):
            assert layer == 1 and ctype == CTYPE_DIRECTIONAL
            assert max(abs(x - 50.0), abs(y - 50.0)) == pytest.approx(ladder[i // 8])
            for off in (x - 50.0, y - 50.0):     # every rung is a multiple of the 0.2 mm grid
                assert abs(off / 0.2 - round(off / 0.2)) < 1e-6
