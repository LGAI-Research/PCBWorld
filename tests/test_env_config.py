...
def test_from_namespace_matches_legacy_mapper():
    ...
    assert got == {
        ...
        "outline_obs": "arc", "simplify_outline": False,
        "action_history_len": 1, "net_constraint_obs": True,
...
def test_from_checkpoint_defaults_on_empty():
    ...
    # (configs/pcbworld.yaml (env:) = the training defaults), so eval matches train.
    got = RLEnvConfig.from_checkpoint({}, max_steps=128).to_pool_kwargs()
    assert got == {
        "max_steps": 128, "masking_rule": "pcbworld_masking",
        "reward_rule": "pcbworld_reward", "force_walkaround": False,
