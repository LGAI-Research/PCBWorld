...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    # Cadagent-style episode metrics:
    #   success_rate               →  mean only (flat scalar)
    #   *_metric                   →  ``_metric`` stripped from display name
    #     all rollouts             →  mean + std  (grouped under same prefix)
    #     success-only rollouts    →  success_{name}/{mean,max,min}
    # Success mask is derived from the success_rate key (1 → success, 0 → fail).
    ntb = batch.non_tensor_batch
    success_key = next(
        (k for k in ntb.keys() if k == "success_rate" or "success_rate" in k),
        None,
    )
    success_mask = None
    if success_key is not None:
        sr = np.asarray(ntb[success_key])[unique_idx]
        metrics[f"episode/{success_key}"] = float(sr.mean())
        success_mask = sr >= 0.5

    for k, v in ntb.items():
        if not k.endswith("_metric"):
            continue
        display = k[: -len("_metric")]
        arr = np.asarray(v)[unique_idx]
        metrics[f"episode/{display}/mean"] = float(arr.mean())
        metrics[f"episode/{display}/std"]  = float(arr.std())
        if success_mask is not None and success_mask.any():
            sub = arr[success_mask]
            metrics[f"episode/success_{display}/mean"] = float(sub.mean())
            metrics[f"episode/success_{display}/max"]  = float(sub.max())
            metrics[f"episode/success_{display}/min"]  = float(sub.min())
