...
To merge a LoRA-finetuned actor by combining the saved adapter with its
base model (skips the FSDP shard reconstruction entirely — uses the
``lora_adapter/`` subfolder verl writes alongside the actor):
```sh
python scripts/model_merger.py merge \
    --merge_lora \
    --backend fsdp \
    --local_dir checkpoints/.../global_step_N/actor \
    --target_dir /path/to/merged_hf_model
    # --hf_model_path optional; defaults to adapter_config.json's
    # base_model_name_or_path
```
``--backend`` is still required by the parser but is ignored when
``--merge_lora`` is set.
...
def _resolve_lora_adapter_dir(local_dir: str) -> str:
    """Locate the PEFT adapter directory next to a verl actor checkpoint.

    verl-agent's ``fsdp_workers._save_checkpoint`` writes the adapter to
    ``<local_dir>/lora_adapter/`` when ``lora_rank > 0``. We also accept
    ``local_dir`` itself already being that adapter folder, for users
    who unpacked / moved the file tree manually.
    """
    candidates = [
        os.path.join(local_dir, "lora_adapter"),
        local_dir,
    ]
    for c in candidates:
        if (os.path.isfile(os.path.join(c, "adapter_config.json"))
                and os.path.isfile(os.path.join(c, "adapter_model.safetensors"))):
            return c
    raise FileNotFoundError(
        f"could not find a PEFT adapter under {local_dir!r}; expected "
        f"either {local_dir}/lora_adapter/ or the path itself to contain "
        f"adapter_config.json + adapter_model.safetensors"
    )


def _merge_lora_and_save(args) -> None:
    """``--merge_lora`` entry point.

    Bypasses FSDP/Megatron shard reconstruction entirely. Loads the
    base HF model (resolved from ``adapter_config.json`` or
    ``--hf_model_path``), wraps it with the saved LoRA adapter, calls
    :meth:`PeftModel.merge_and_unload`, and saves the merged HF
    checkpoint to ``--target_dir``. Tokenizer is also re-saved so the
    output directory is a self-contained vLLM-loadable model.
    """
    import json
    from peft import PeftModel
    from transformers import AutoModelForCausalLM, AutoTokenizer

    adapter_dir = _resolve_lora_adapter_dir(args.local_dir)
    print(f"[merge_lora] adapter   : {adapter_dir}")

    with open(os.path.join(adapter_dir, "adapter_config.json")) as f:
        adapter_cfg = json.load(f)
    base_path = args.hf_model_path or adapter_cfg.get("base_model_name_or_path")
    if not base_path:
        raise ValueError(
            "could not determine base model: adapter_config.json has no "
            "base_model_name_or_path and --hf_model_path was not supplied"
        )
    print(f"[merge_lora] base model: {base_path}")

    print("[merge_lora] loading base (bf16) — this may take a while…")
    base = AutoModelForCausalLM.from_pretrained(
        base_path, torch_dtype=torch.bfloat16, trust_remote_code=True,
    )

    print("[merge_lora] attaching adapter…")
    ...
    print(f"[merge_lora] saving merged model to {target}")
    merged.save_pretrained(target, safe_serialization=True)

    # Tokenizer: prefer the one shipped next to the actor (carries any
    # custom chat-template tweaks), fall back to the base model.
    tok_source = args.local_dir if os.path.isfile(
        os.path.join(args.local_dir, "tokenizer_config.json")
    ) else base_path
    print(f"[merge_lora] saving tokenizer from {tok_source}")
    AutoTokenizer.from_pretrained(
        tok_source, trust_remote_code=True,
    ).save_pretrained(target)
    print(f"[merge_lora] done → {target}")
...
def main():
    ...
    merge_parser.add_argument(
        ...
        help="Merge the PEFT/LoRA adapter saved under "
             "``<local_dir>/lora_adapter/`` into its base model (resolved "
             "from adapter_config.json or --hf_model_path) and save the "
             "result to --target_dir. Bypasses FSDP/Megatron shard "
             "reconstruction; --backend is still required by the parser "
             "but ignored.",
    )
    ...
    # LoRA merge takes precedence and skips the FSDP/Megatron path.
    if args.operation == "merge" and getattr(args, "merge_lora", False):
        _merge_lora_and_save(args)
        return
