...
#   - Updated make_envs() to handle "pcbworld" env_name
# ---------------------------------------------------------------------------
...
# Trainer-agnostic pcbworld rollout adapter base. Lives under methods/llm_agent/training
# so the pcbworld-specific glue (memory, v5 feedback streak, prompt assembly)
# is reusable by other training frameworks and survives publishing without
# this verl-agent-patch directory.
...
# ===========================================================================
# KiCadLLMVerlManager  (NEW)
# ===========================================================================

class KiCadLLMVerlManager(KiCadLLMRolloutManager, EnvironmentManagerBase):
    """verl-agent adapter for the KiCad PCB routing environment.

    PCBWorld-specific state and per-step rollout logic — SimpleMemory,
    ...
    pieces:
      ...
      - ``_post_step_info`` stamps ``to_numpy(valids[i])`` onto each info as
        ``'is_action_valid'`` (verl-agent's torch/np contract).
      - ``_process_batch`` extracts the pcbworld 6-metric panel from final
    ...
    Action candidates (7 total, 6 LLM-facing + 1 fallback):
        0 net_select    {"action_type": 0, "net_id": <int>}
        1 start_route   {"action_type": 1, "x_mm": <f>, "y_mm": <f>, "layer": <int>}
        2 net_end       {"action_type": 2}
        3 make_line     {"action_type": 3, "x_mm": <f>, "y_mm": <f>, "routing_mode": <int>}
        4 make_via      {"action_type": 4, "x_mm": <f>, "y_mm": <f>, "routing_mode": <int>}
        5 finish        {"action_type": 5, "routing_mode": <int>}
        6 idle          {"action_type": 6}  — fallback for unparseable LLM output

    Routing modes (LLM uses letters, env uses ints):
        m (0) MarkObstacles  — fastest, no re-routing
        p (1) PushAndShove   — pushes existing traces aside
        w (2) Walkaround     — avoids obstacles (default / safest)
    """

    def __init__(
        self, envs, projection_f, config, board_scheduler=None,
        score_episodes=False,
    ):
        # MRO is (PCBWorld..., KiCadLLMRolloutManager, EnvironmentManagerBase, object).
        # ``EnvironmentManagerBase.__init__`` only stores envs/projection_f/config
        # which KiCadLLMRolloutManager already does, so call the pcbworld base
        # explicitly and skip the verl-agent base init.
    ...
    def step(self, text_actions: List[str]) -> Tuple[Dict[str, Any], Any, Any, List[Dict]]:
        # Reuse the base rollout logic, then coerce rewards/dones into the
        # numpy contract verl-agent's trainer expects (the pcbworld base
        # has no torch/numpy dependency by design).
        next_observations, rewards, dones, infos = super().step(text_actions)
        return next_observations, to_numpy(rewards), to_numpy(dones), infos

    # ------------------------------------------------------------------
    # Success evaluation (pcbworld 6-metric panel)
    # ------------------------------------------------------------------

    def _process_batch(self, batch_idx, total_batch_list, total_infos, success):
        """Evaluate routing success from a completed episode.

        Metrics recorded:
          success_rate          — 1.0 if all nets routed, 0.0 otherwise
          drc_violations_metric — number of DRC violations at episode end
          wirelength_metric     — total wirelength (mm) at episode end
          via_count_metric      — number of vias placed at episode end
          routability_metric    — connected / total targets at episode end
                                  (0.0–1.0; 1.0 == fully routed)
          track_count_metric    — number of tracks placed at episode end
        """
        for i in reversed(range(len(total_batch_list[batch_idx]))):
            batch_item = total_batch_list[batch_idx][i]
            if batch_item['active_masks']:
                ...
                success['success_rate'].append(won_value)
                success['drc_violations_metric'].append(float(info.get('drc_violations', 0)))
                success['wirelength_metric'].append(float(info.get('wirelength', 0.0)))
                success['via_count_metric'].append(float(info.get('via_count', 0)))
                success['routability_metric'].append(float(info.get('routability', 0.0)))
                success['track_count_metric'].append(float(info.get('track_count', 0)))
                return


# ===========================================================================
# make_envs factory  (original + pcbworld branch)
# ===========================================================================
...
def make_envs(config):
    ...
    if "search" in config.env.env_name.lower():
        ...
        return envs, val_envs
    elif "pcbworld" in config.env.env_name.lower():
        from configs.loader.schema import LLMTrainConfig
        from methods._shared.board_scheduler import (
            BoardScheduler,
            BoardSchedulerConfig,
        )
        from methods.llm_agent.wrappers.factory import build_pcbworld_envs
        from methods.llm_agent.wrappers.action_converter import pcbworld_projection

        # pcbworld-side config slice (the env.pcbworld.* block + the env.*
        # knobs pcbworld reads). Defaults live in configs/pcbworld.yaml (llm_train:)
        ...
        train_scheduler = BoardScheduler(BoardSchedulerConfig(
            mode=boards_order,
            single_board=board_path,
            boards_json=cad.boards_json,
            difficulty=cad.boards_difficulty,
        ...
        if val_boards_order == 'single':
            val_scheduler = BoardScheduler(BoardSchedulerConfig(
                mode='single',
                single_board=board_path,
            ))
        else:
            val_scheduler = BoardScheduler(BoardSchedulerConfig(
                mode=val_boards_order,
                single_board=board_path,
                boards_json=cad.boards_json,
                difficulty=cad.boards_difficulty,
                split=cad.resolved_val_boards_split,
                # Distinct seed so val's per_env_epoch shuffle doesn't track
                # train's order when both are enabled.
                seed=cad.seed + 1000,
            ))

        initial_train_paths = train_scheduler.next_paths(
            config.data.train_batch_size
        )
        initial_val_paths = val_scheduler.next_paths(config.data.val_batch_size)

        _envs = build_pcbworld_envs(
            board_paths=initial_train_paths,
        ...
        _val_envs = build_pcbworld_envs(
            board_paths=initial_val_paths,
        ...
        print(
            f"[pcbworld] boards_order={boards_order} "
            f"train_boards={train_scheduler.num_boards} "
            f"val_boards={val_scheduler.num_boards}"
        )

        projection_f = partial(pcbworld_projection)
        # Episode-end canonical DRC scoring: always on for the val manager
        # (feeds the central val/* metrics), opt-in for train rollouts
        # (per-episode DRC cost).
        envs = KiCadLLMVerlManager(
            _envs, projection_f, config, board_scheduler=train_scheduler,
            score_episodes=cad.score_train_episodes,
        )
        val_envs = KiCadLLMVerlManager(
            _val_envs, projection_f, config, board_scheduler=val_scheduler,
            score_episodes=True,
        )
        return envs, val_envs
    else:
