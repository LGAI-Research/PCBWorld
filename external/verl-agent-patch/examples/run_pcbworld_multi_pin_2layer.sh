...
# W&B logging guard (see run_pcbworld.sh): drop to console-only when
...
if [[ -z "${LOGGER:-}" ]]; then
    if [[ -n "${WANDB_API_KEY:-}" ]]; then
        ...
        echo "[run_pcbworld] WANDB_API_KEY unset -> console-only logging" >&2
        LOGGER="['console']"
    fi
fi
WANDB_PROJECT_NAME="${WANDB_PROJECT:-verl_agent_pcbworld}"

# Multi-pin 2-layer synthetic experiment.
# Uses experiments/kdd/configs/datasets/misc/multi_pin_2layer_v2.json (10k train + 128 test + 128 val
...
BOARDS_JSON=${BOARDS_JSON:-"$REPO_ROOT/experiments/kdd/configs/datasets/misc/multi_pin_2layer_v2.json"}
BOARDS_DIFFICULTY=${BOARDS_DIFFICULTY:-easy}
BOARDS_SPLIT=${BOARDS_SPLIT:-train}

# Single-board fallback (used by env.pcbworld.board_path init; round-robin
...
# => PCBWorldEnvs.num_processes = 8 * 8 = 64 workers
train_data_size=8
val_data_size=3
group_size=8
...
python3 -m verl.trainer.main_ppo \
    ...
    env.pcbworld.board_path=$BOARD_PATH \
    +env.pcbworld.boards_order=$BOARDS_ORDER \
    +env.pcbworld.boards_json=$BOARDS_JSON \
    +env.pcbworld.boards_difficulty=$BOARDS_DIFFICULTY \
    +env.pcbworld.boards_split=$BOARDS_SPLIT \
    +env.pcbworld.val_boards_order=round_robin \
    +env.pcbworld.val_boards_split=val \
    env.pcbworld.masking_rule=strict \
    env.pcbworld.reward_rule=grpo_final \
    env.pcbworld.state_format=sexpr \
    env.pcbworld.prompt_version=v5 \
