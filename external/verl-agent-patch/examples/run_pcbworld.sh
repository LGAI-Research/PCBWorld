...
#   LOGGER="['console']" bash run_pcbworld.sh
if [[ -z "${LOGGER:-}" ]]; then
...
# Defaults to tests/fixtures/simple_routing_board.kicad_pcb in the pcbworld repo root.
# Override via: BOARD_PATH=/path/to/board.kicad_pcb bash run_pcbworld.sh
...
#   BOARDS_ORDER=round_robin BOARDS_DIFFICULTY=easy BOARDS_SPLIT=train bash run_pcbworld.sh
...
python3 -m verl.trainer.main_ppo \
    ...
    +env.pcbworld.boards_order=${BOARDS_ORDER:-single} \
    +env.pcbworld.boards_json=${BOARDS_JSON:-$REPO_ROOT/pcb_dataset_difficulty.json} \
    +env.pcbworld.boards_difficulty=${BOARDS_DIFFICULTY:-easy} \
    +env.pcbworld.boards_split=${BOARDS_SPLIT:-train} \
    env.pcbworld.masking_rule=strict \
    env.pcbworld.reward_rule=grpo_final \
    env.pcbworld.state_format=sexpr \
    env.pcbworld.prompt_version=v4 \
