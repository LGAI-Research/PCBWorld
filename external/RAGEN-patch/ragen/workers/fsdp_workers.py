...
from verl.utils.fsdp_utils import (
    ...
    get_shard_placement_fn,  # missing in RAGEN original; needed for fsdp2 critic
    init_fn,
)
...
def build_peft_model(module, model_config, task_type, modules_to_save=None):
    """Apply a standard LoRA adapter to *module* (mirrors verl's native LoRA support).

    verl only supports standard LoRA via LoraConfig. Reads the following keys
    from *model_config*:
      lora_rank        – adapter rank
      lora_alpha       – LoRA scaling factor
      target_modules   – modules to adapt (string or list)
      exclude_modules  – modules to exclude from adaptation (optional)

    Args:
      modules_to_save: additional module names to clone and train alongside the
        adapter (e.g. ["score"] for critic value head).
    """
    lora_rank = model_config.lora_rank
    lora_alpha = model_config.lora_alpha
    target_modules = convert_to_regular_types(model_config.target_modules)
    exclude_modules = convert_to_regular_types(model_config.get("exclude_modules", None))
...
class CriticWorker(VerlCriticWorker):
    def _build_critic_model_optimizer(self, config):
        ...
        if self._is_lora:
            # modules_to_save=["score"] ensures the value-head is cloned and
            # trained alongside the LoRA adapter (must be set at get_peft_model time).
            critic_module = build_peft_model(
                critic_module, self.config.model,
