...
#
#   QS_REPO_ROOT                          — absolute path to the pcbworld repo.
...
qs_load_env() {
    ...
    if [ -n "$_certifi" ] && [ -f "$_certifi" ]; then
        if [ -z "${SSL_CERT_FILE:-}" ] || [ "$SSL_CERT_FILE" = "$_certifi" ]; then
            ...
            local _bundle="$HOME/.cache/pcbworld/ssl-ca-bundle.pem"
...
qs_models_cfg_path() {
    echo "${QS_MODELS_CONFIG:-$QS_REPO_ROOT/experiments/kdd/configs/quickstart/models.json}"
}

qs_splits_cfg_path() {
    echo "${QS_SPLITS_CONFIG:-$QS_REPO_ROOT/experiments/kdd/configs/quickstart/splits.json}"
}
