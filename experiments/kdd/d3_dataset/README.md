...
Rebuild [`configs/datasets/d3.json`](../../../configs/datasets/d3.json) — the D3 boards-json consumed by the eval split aliases (built from the `pcbench_exacad`
corpus: the paper's files, or a rebuild of them by `tools/datagen/pcbench_prep/`):
...
* `T3_SORTED_DIR` — default for `--sorted-dir`
  (fallback `$PCBWORLD_DATA_ROOT/pcbench/exacad_sorted`).
...
* Quickstart split aliases: [`experiments/kdd/configs/quickstart/splits.json`](../configs/quickstart/splits.json)
