...
# Both bypass the PCBWorldEnvs step loop — the LLM never observes intermediate
...
# activate pcbworld externally and just source/run this script.
...
if [ -f "$_conda_setup" ] && [ "${CONDA_DEFAULT_ENV:-}" != "pcbworld" ]; then
...
# Defaults resolve through configs/paths.yaml (PCBWORLD_DATA_ROOT overrides the
