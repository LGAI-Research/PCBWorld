...
#
# Drives ``methods/llm_agent/rollout/pcbworld.py --mode api`` on a named benchmark
# split with a named model alias. The alias → provider/model and split →
# boards-json mappings are loaded from JSON configs in
# ``experiments/kdd/configs/quickstart/`` by default; override per-call with --config-models
...
#
# Flags:
#   --model <alias>           (required)  see experiments/kdd/configs/quickstart/models.json
#   --split <alias>           (required)  see experiments/kdd/configs/quickstart/splits.json
...
#   --extra-args "<...>"      raw flags forwarded to the pcbworld rollout
...
if [ "$LIMIT" != "0" ]; then
    QS_LIMITED_BOARDS_JSON="$(mktemp "/tmp/pcbworld_qs_${SPLIT_ALIAS}_XXXXXX.json")"
...
py_cmd=(
    python -m methods.llm_agent.rollout.pcbworld
    --mode api
...
# Post-process: convert the pcbworld rollout's flat dump ($OUT/interactive/_raw/
...
raw = out / "_raw"
if not raw.is_dir():
    sys.exit(0)  # nothing to do (e.g. the pcbworld rollout never wrote)
