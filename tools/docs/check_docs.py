#!/usr/bin/env python3
"""Deterministic doc/code consistency checker for pcbworld (Tier-1 doc-sync).
...
Checks (run all by default; `--only <name>` to run one):
  ...
  public-hygiene — no public file imports or names anything under internal/ (names derived
                 ...
                 (internal/hygiene_patterns.txt); dataset configs only under ${PCBWORLD_DATA_ROOT}
...
DEAD_PATHS: list[tuple[str, str]] = [
    ...
    ("experiments/kdd/configs/datasets/misc/pcb_dataset_difficulty.json",
    ...
    ("envs.llm_env", "moved to methods.llm_agent.* (eval_cadagent → methods.llm_agent.rollout.pcbworld)"),
    ("policy/llm", "moved to methods/llm_agent/policy/"),
    ("training/llm", "moved to methods/llm_agent/training/"),
    ("eval/rollout/cadagent", "moved to methods/llm_agent/rollout/pcbworld.py"),
    ("eval/rollout/apiseq", "moved to methods/llm_agent/rollout/apiseq.py"),
    ("eval_cadagent", "renamed to methods/llm_agent/rollout/pcbworld.py (module methods.llm_agent.rollout.pcbworld)"),
    # 2026-09-11 product-name rename cadagent → pcbworld (conda env, PCBWORLD_* env vars, modules, config keys, prompt bundles).
    ("methods/llm_agent/rollout/cadagent.py", "renamed to methods/llm_agent/rollout/pcbworld.py"),
    ("methods.llm_agent.rollout.cadagent", "renamed to methods.llm_agent.rollout.pcbworld"),
    ("tests/test_eval_cadagent_canonical.py", "renamed to tests/test_eval_pcbworld_canonical.py"),
    ("run_cadagent", "verl-agent examples renamed to run_pcbworld*.sh"),
    ("get_cadagent_prompts", "renamed to get_pcbworld_prompts"),
    ("CADAGENT_", "every CADAGENT_* environment variable is now PCBWORLD_* (no fallback)"),
    ("~/.cadagent", "the internal data-root symlink tree is ~/.pcbworld (re-run bash internal/setup.sh)"),
    ...
    ("docs/INCREMENTAL_DRC.md", "moved to internal/docs/design/INCREMENTAL_DRC.md (summary: internal/docs/ENGINE.md §5)"),
    # Legacy masking YAML removal — 4 unloadable-by-file variants removed; the names still map via _NAME_COMPAT.
    ("configs/masking/strict.yaml", "removed — name-only clone of experiments/kdd/configs/masking/default.yaml (name 'strict' still maps via _NAME_COMPAT)"),
    ("configs/masking/strict_no_finish.yaml", "removed — name-only clone of default_no_finish.yaml (now experiments/kdd/configs/masking/; name still maps via _NAME_COMPAT)"),
    ...
    # 2026-09-10 config split — the d2b ablation rules were promoted into configs/ (the KDD rules moved to
    # experiments/kdd/configs/ are not listed: this is a substring check and would match their new paths).
    ("tests/fixtures/reward_rules/reward.yaml", "promoted to internal/experiments/d2b_midboard/reward/drc_dense_promoted_invpen01.yaml"),
    ("internal/experiments/d2b_midboard/masking_netfree.yaml", "promoted to internal/experiments/d2b_midboard/masking/netend_free.yaml"),
    ("internal/experiments/d2b_midboard/masking_netfree_nofinish.yaml", "promoted to configs/masking/pcbworld_masking.yaml (ex netend_free_no_finish)"),
    ("internal/experiments/d2b_midboard/reward.yaml", "promoted to internal/experiments/d2b_midboard/reward/drc_dense_promoted_invpen01.yaml"),
    ...
    # configs/ top level is data-only (defaults.yaml datasets/ drc/ masking/ reward/ paths.yaml).
    ("configs/defaults/", "the six per-surface files were merged into configs/pcbworld.yaml (sections env/rl_train/rl_policy/rl_eval/llm/llm_train)"),
    ("configs/defaults.yaml", "renamed to configs/pcbworld.yaml"),
    ...
    # (configs/quickstart/{models,splits}.json entries dropped: substrings of the live experiments/kdd/configs/quickstart/ paths)
    ...
    # MCTS moved under internal/ as a whole (it does not ship in public releases).
    # Note: the old path is a SUBSTRING of the new one (`internal/methods/_shared/mcts`), and
    # dead-paths is a `git grep -F` literal scan, so the path-prefix form cannot be guarded
    # (it would match itself). Only the import forms are registered instead: they are what
    # actually breaks code, and the `internal.` prefix makes those strings disappear.
    ("from methods._shared.mcts", "moved to internal.methods._shared.mcts"),
    ("import methods._shared.mcts", "moved to internal.methods._shared.mcts"),
    ("from methods.rl_agent.policy.mcts_", "moved to internal.methods.rl_agent.policy."),
    ("import methods.rl_agent.policy.mcts_", "moved to internal.methods.rl_agent.policy."),
    ("m methods.rl_agent.policy.mcts_", "python -m: use internal.methods.rl_agent.policy."),
    ("search/mcts", "core moved to internal/methods/_shared/mcts/ (branch-agnostic); mcts_compare.py -> internal/methods/rl_agent/policy/"),
    ("search.mcts", "moved to internal.methods._shared.mcts (compare: internal.methods.rl_agent.policy.mcts_compare)"),
    ...
    ("t3_pcb_dataset", "renamed to d3.json (t3→d3); now configs/datasets/d3.json"),
...
DEAD_PATH_EXCLUDES = [
    ...
    # embed a dead-path substring, e.g. a test named after an old module); ghost entries are
    # caught by check_test_durations.
...
def _private_forms(root: Path) -> dict[str, str]:
    ...
    forms = {re.escape(PRIVATE_ROOT): "the private root",
             r"(?<![\w.])internal\.(?=[A-Za-z_])": "the private root (dotted import form)",
    ...
    ls = subprocess.run(["git", "-C", str(root), "ls-files"], capture_output=True, text=True)
    tracked = ls.stdout.splitlines()
    # A public mirror "exists" when git TRACKS a file under that path — never by looking at the
    # disk: an untracked leftover (a stale __pycache__/ under a moved package) would otherwise
    # pass for a mirror and turn the moved file's basename into a forbidden word.
    public_paths: set[str] = set()
    for rel in tracked:
        if rel.startswith(PRIVATE_ROOT):
            continue
        parts = rel.split("/")
        for i in range(1, len(parts) + 1):
            public_paths.add("/".join(parts[:i]))
    leaves: dict[str, bool] = {}
    for rel in tracked:
        if not rel.startswith(PRIVATE_ROOT):
            continue
        parts = rel.split("/")[1:]
        for i in range(1, len(parts) + 1):
            cand = "/".join(parts[:i])
            if cand not in public_paths:
...
def _git_grep(root: Path, pattern: str, files: list[str]) -> list[tuple[str, int, str]] | None:
    """`git grep -nIP` over *files* → [(rel, lineno, text)]; None when PCRE rejects the pattern
    (the caller then falls back to the Python scan). One subprocess per rule instead of a Python
    regex over every line of every public file (~90 s → <1 s on this tree)."""
    if not files:
        return []
    proc = subprocess.run(["git", "-C", str(root), "grep", "-nIP", "--no-color", "-e", pattern, "--", *files],
    ...
    for line in proc.stdout.splitlines():
        ...
        hits.append((rel, int(lineno), text))
    return hits
...
def check_public_hygiene(root: Path) -> list[str]:
    ...
    public = [rel for rel in proc.stdout.splitlines()
              if not (rel.startswith(PRIVATE_ROOT) or rel.startswith(ROOT_EXCLUDED) or rel.startswith(PUBLIC_HYGIENE_SKIP))]
    failures: list[str] = []
    for rel in public:
    ...
    for rel in public:
        if Path(rel).suffix in _HYGIENE_BINARYISH:
            continue
        try:
            if (root / rel).stat().st_size > 2_000_000:
                continue
        except OSError:
            continue
        scan.append(rel)

    # Rule 1: no public file names a private path (import statement or path string).
    hits = _git_grep(root, "|".join(f"(?:{p})" for p in pats), scan)
    if hits is None:
        return _check_public_hygiene_python(root, forms, ident, allow, scan)
    compiled = [re.compile(p) for p in pats]
    for rel, lineno, text in hits:
        for i, rx in enumerate(compiled):
            m = rx.search(text)
            if m:
                failures.append(f"{rel}:{lineno}: public file names private path '{m.group(0)}' ({names[i]}) — public "
                                "code must not import or reference internal/; move the consumer under internal/ or cut the dependency")
                break
    # Rule 2: the public set is English-only (code comments, strings, and the docs that ship).
    # The class is built from code points so this public file carries no Hangul bytes itself
    # (the clean-room byte audit greps every shipped text file).
    hits = _git_grep(root, "[" + chr(0xAC00) + "-" + chr(0xD7A3) + chr(0x3131) + "-" + chr(0x318E) + "]", scan)
    if hits is None:
        return _check_public_hygiene_python(root, forms, ident, allow, scan)
    korean: dict[str, int] = {}
    for rel, _, _ in hits:
        korean[rel] = korean.get(rel, 0) + 1
    for rel, n in korean.items():
        failures.append(f"{rel}: {n} line(s) contain Korean — the public set is English-only; translate, or move the file under internal/")
    # Rule 3: identity / infrastructure seeds (allow: literals stripped before the match).
    if ident is not None:
        hits = _git_grep(root, ident.pattern, scan)
        if hits is None:
            return _check_public_hygiene_python(root, forms, ident, allow, scan)
        for rel, lineno, text in hits:
            probe = text
            ...
            if hit:
                failures.append(f"{rel}:{lineno}: identity/infra seed '{hit.group(0)}' (rule 3 — the public set carries no "
                                f"internal names, hosts or paths; see {HYGIENE_PATTERNS})")
    failures += _check_config_data_roots(root)
    return failures


def _check_public_hygiene_python(root: Path, forms: dict[str, str], ident, allow: list[str], scan: list[str]) -> list[str]:
    """Fallback when PCRE rejects a pattern: the original line-by-line Python scan (slow, same rules)."""
    ...
    for rel in scan:
        ...
        for lineno, line in enumerate(text.splitlines(), 1):
            for m in regex.finditer(line):
                ...
                failures.append(f"{rel}:{lineno}: public file names private path '{m.group(0)}' ({names[idx]}) — public "
                                "code must not import or reference internal/; move the consumer under internal/ or cut the dependency")
            ...
            if ident is not None:
                ...
                if hit:
                    failures.append(f"{rel}:{lineno}: identity/infra seed '{hit.group(0)}' (rule 3 — the public set carries no "
                                    f"internal names, hosts or paths; see {HYGIENE_PATTERNS})")
        if korean:
            failures.append(f"{rel}: {korean} line(s) contain Korean — the public set is English-only; translate, or move the file under internal/")
    failures += _check_config_data_roots(root)
    return failures
...
def _check_config_data_roots(root: Path) -> list[str]:
    """Rule 4: shipped dataset configs name locations only as ${PCBWORLD_DATA_ROOT}/<sub>
    ...
    for p in sorted([*(root / "configs" / "datasets").rglob("*.json"),
                     *(root / "experiments" / "kdd" / "configs" / "datasets").rglob("*.json")]):
        try:
            ...
            if isinstance(node, dict):
                ...
                if node.startswith("/"):
                    failures.append(f"{p.relative_to(root)}: absolute dataset path under '{key}' "
                                    f"({node[:60]}…) — use ${{PCBWORLD_DATA_ROOT}}/<sub> (rule 4)")
        walk(doc)
...
def check_tracked_ignored(root: Path) -> list[str]:
    """The reverse of gitignore-traps: nothing tracked may sit under an ignore rule, and no
    symlink may be tracked. Both leak local layout into the repo — a worktree's `var ->
    ../main/var` link is a file to git, so a directory-only rule (`/var/`) never matched it and
    `git add -A` committed it once. Generated trees (var/, build_rl/,
    sandbox/) are data or machine-local links; the repo carries neither."""
    failures = []
    ign = subprocess.run(["git", "ls-files", "-i", "-c", "--exclude-standard"], cwd=root,
                         capture_output=True, text=True)
    if ign.returncode != 0:
        return [f"git ls-files -i failed: {ign.stderr.strip()}"]
    for path in ign.stdout.splitlines():
        failures.append(f"{path}: tracked but matches an ignore rule — `git rm --cached {path}` "
                        f"(the rule says this is generated/local; if it is source, fix the rule instead)")
    ls = subprocess.run(["git", "ls-files", "-s"], cwd=root, capture_output=True, text=True, check=True).stdout
    for line in ls.splitlines():
        meta, _, path = line.partition("\t")
        if meta.split()[0] == "120000":
            failures.append(f"{path}: tracked symlink — `git rm --cached {path}` (symlinks are machine-local "
                            f"layout; a directory-only ignore rule like `/{path}/` does not cover them)")
    return failures
...
CHECKS = {
    ...
    "tracked-ignored": check_tracked_ignored,
