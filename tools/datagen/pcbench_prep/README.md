...
export PCBWORLD_DATA_ROOT=/data/pcbworld                 # your dataset root
...
the rebuilt set). Compared file by file after removing what KiCad mints afresh on every convert
...
therefore reproduces the set everywhere except those boards; the shipped `d3` is the
engine-CLI output.
...
the only remaining difference. The rebuilt set ships as the logical dataset `d3`
([configs/datasets/d3.json](../../../configs/datasets/d3.json), dataset dir
`pcbench_exacad`); the paper's numbers were measured on the paper's own files; the rebuilt set replaces them
file for file under the same `pcbench/exacad_sorted` directory.
