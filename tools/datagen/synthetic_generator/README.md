...
| [setup_10net_2pin_1layer_split_v2.py](setup_10net_2pin_1layer_split_v2.py) | `pcb_dataset_10net_2pin_1layer_combined_v2/` + `experiments/kdd/configs/datasets/grids/10net_2pin_1layer_v2.json` | Symlinks the `_v2` train (10K) + test (128) sets, renaming test boards to `testboard_NNNNN`. |
| [setup_multi_pin_synthetic_split_v2.py](setup_multi_pin_synthetic_split_v2.py) | `pcb_dataset_multi_pin_2layer_combined_v2/` + `experiments/kdd/configs/datasets/misc/multi_pin_2layer_v2.json` | Same as above for the 2L side. Train is `_v2` (a copy trimmed to the first 10K). |
...
Two datasets under `$PCBWORLD_DATA_ROOT/` (and their repo-local
...
`experiments/kdd/configs/datasets/grids/10net_2pin_1layer_v2.json` and
`experiments/kdd/configs/datasets/misc/multi_pin_2layer_v2.json`.
...
# D2-B family (engine required: conda pcbworld + PYTHONPATH=build_rl/pcbnew/python/rl:.)
