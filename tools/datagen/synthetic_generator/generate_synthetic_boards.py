...
# Poisson(k * N_ref^2) with N_ref = side / ref_pitch, and pins per net from a
...
def _sample_d2b_pins_per_net(rng: random.Random, n_nets: int,
...
def _clip_pins_per_net(pins_per_net: list[int], cap: int) -> list[int]:
    ...
    carry few/no high-pin-count rails."""
    ppn = list(pins_per_net)
    total = sum(ppn)
    if total <= cap:
        return ppn
    for i in sorted(range(len(ppn)), key=lambda j: -ppn[j]):
        if total <= cap:
            break
        take = min(ppn[i] - 2, total - cap)
        if take > 0:
            ppn[i] -= take
            total -= take
    while total > cap and len(ppn) > 2:
        total -= ppn.pop()
    return ppn
...
def generate_one_d2b(seed: int, num_layers: int, central_frac: float,
    ...
    pads_per_net = _sample_d2b_pins_per_net(rng, n_nets, pr)
    ...
    if pr.rule_mode == "uniform":
        ...
        pads_per_net = _clip_pins_per_net(pads_per_net, _geo_capacity(
...
def generate_one_paired(seed: int, num_layers: int, central_frac: float,
    ...
    Identical board size, net count, pins per net, pad (x,y) positions, thru-hole
...
def _run_d2b(args) -> None:
    ...
    print(f"  pins/net: {1-pr.rail_prob:.0%} bulk({pr.bulk_base}+Poisson({pr.bulk_lambda})) "
