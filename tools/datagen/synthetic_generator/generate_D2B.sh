...
#   pins/net: 90% (3+Poisson(0.35)) + 10% rail lognormal(med 13, s 0.5)[8,32]
...
#
# Requires the engine importable (conda pcbworld + PYTHONPATH=build_rl/...)
...
python -c 'import pcb_world.engine' 2>/dev/null \
  || { echo "ERROR: engine not importable — conda pcbworld + PYTHONPATH=build_rl/pcbnew/python/rl:. required" >&2; exit 1; }
