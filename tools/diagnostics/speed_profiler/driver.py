...
def build_trainer(cfg: ProfileConfig, scratch: str):
    ...
    # d3_v2.json only has train/test (no val) → branch on eval-split.
    ...
    if cfg.dataset == "d2a":
        boards_json, difficulty, boards_split, eval_split = (
            "experiments/kdd/configs/datasets/d2a.json", "easy", "train", "val")
    ...
    elif cfg.dataset.endswith(".json"):
        # any split JSON, e.g. one under the gitignored configs/datasets/local/
        boards_json, difficulty, boards_split, eval_split = (
            cfg.dataset, "easy", "train", "val")
        if not os.path.exists(os.path.join(_REPO_ROOT, boards_json)):
            raise SystemExit(f"split json not found: {boards_json}")
    else:
        raise SystemExit(f"bad dataset {cfg.dataset} — d2a | d2b | d3b, or a split-JSON path")

    argv = [
        "--board", "tests/fixtures/simple_routing_board.kicad_pcb",
        "--boards-order", "per_env_epoch", "--boards-json", boards_json,
        "--use-yaml-drc-fallback", "--drc-config-path", "experiments/kdd/configs/drc/synth_2L_v2.yaml",
...
def run(cfg: ProfileConfig) -> dict:
    ...
    if cfg.enable_barrier or cfg.enable_rollout_decomp:
        os.environ["PCBWORLD_PROFILE_WORKER"] = "1"
        import multiprocessing as mp
        try:
    ...
    # file/dir names carry the dataset: a split-JSON path collapses to its stem
    ds_tag = os.path.splitext(os.path.basename(cfg.dataset))[0]
    scratch = os.path.join(_REPO_ROOT, "var", "diagnostics", "speed_profiler",
                           f"{ds_tag}_e{cfg.n_envs}")
    os.makedirs(scratch, exist_ok=True)

    result: dict = {
        "run": {"host_tag": cfg.host_tag, "host": os.uname().nodename,
                "dataset": ds_tag, "n_envs": cfg.n_envs, "algo": "ppo",
    ...
    print(f"[prof] {ds_tag} e{cfg.n_envs} spawn+setup {result['run']['spawn_setup_s']}s", flush=True)
    ...
    out = cfg.out or os.path.join(scratch, f"prof_{ds_tag}_e{cfg.n_envs}.json")
