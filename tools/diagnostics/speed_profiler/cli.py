...
``var/`` (gitignored). Requires the ``pcbworld`` conda env + PYTHONPATH/LD_LIBRARY_PATH.
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--dataset", default="d2a",
                   help="d2a | d2b | d3b, or the path of a split JSON to profile on "
                        "(read as easy/train with val for eval); the run is tagged with "
                        "its file stem")
