...
- **Single env**: transformer/KiCad rollout AND the rule-based baselines
  (FreeRouting/KRT/OrthoRoute) all run in the `pcbworld` env —
...
> to `$PCBWORLD_DATA_ROOT/KDD_benchmark` (the `kdd_bench` dataset of `configs/paths.yaml`)
...
in-repo `var/` trees this reproduction reads and writes. `PCBWORLD_DATA_ROOT` (used
...
```bash
export PCBWORLD_DATA_ROOT="$PWD/var/datasets"   # dataset corpus (Python resolver)
export DATASET_ROOT="$PCBWORLD_DATA_ROOT"       # same tree, for the dispatch scripts
```
...
via `--stages` of `eval/pipeline.py` (run in the `pcbworld` env with the §0 roots exported).
...
- `<m>_avg` = mean over rollouts, `<m>_best` = the `selection-method` winner rollout's value.
  Metric vocabulary = `eval_utils.PCBWORLD_VALUE_METRIC_FIELDS` (avg/best reuse the same
...
- `table1` / `reward` train on the [d2a split](../experiments/kdd/configs/datasets/d2a.json) (10 000
