...
class PotentialReward:
    ...
    Args:
        ...
        net_bonus_size_linear_scale: Linear twin of
            ``net_bonus_size_log_scale`` — ``w_i = scale * pad_groups_i``
            (no log compression), so closing a big net pays in proportion to
            its size. Mutually exclusive with the log knob.
        completion_bonus_padgroup_scale /
        clean_completion_bonus_padgroup_scale: Board-bonus twins scaled by the
            board's TOTAL bare-board pad groups instead of ``ln(1+net_count)``
            — ``bonus = scale * sum(pad_groups)``, resolved in the per-reset
            group (pad groups are episode-dependent under keep-routing).
            Mutually exclusive with the matching ``*_log_scale`` knob.
    ...
    def __init__(
        ...
        net_bonus_size_linear_scale: float = 0.0,
        ...
        completion_bonus_padgroup_scale: float | None = None,
        clean_completion_bonus_padgroup_scale: float | None = None,
        ...
        if net_bonus_size_linear_scale < 0:
            raise ValueError(
                f"net_bonus_size_linear_scale must be >= 0, "
                f"got {net_bonus_size_linear_scale}."
            )
        if net_bonus_size_log_scale > 0 and net_bonus_size_linear_scale > 0:
            raise ValueError(
                "net_bonus_size_log_scale and net_bonus_size_linear_scale are "
                "mutually exclusive — pick one ladder-weight shape."
            )
        if (completion_bonus_log_scale is not None
                and completion_bonus_padgroup_scale is not None):
            raise ValueError(
                "completion_bonus_log_scale and completion_bonus_padgroup_scale "
                "are mutually exclusive — pick one board-bonus shape."
            )
        if (clean_completion_bonus_log_scale is not None
                and clean_completion_bonus_padgroup_scale is not None):
            raise ValueError(
                "clean_completion_bonus_log_scale and "
                "clean_completion_bonus_padgroup_scale are mutually exclusive."
            )
        ...
        self.net_bonus_size_linear_scale = net_bonus_size_linear_scale
        ...
        self.completion_bonus_padgroup_scale = completion_bonus_padgroup_scale
        self.clean_completion_bonus_padgroup_scale = (
            clean_completion_bonus_padgroup_scale
        )
    ...
    def bind_board(
        ...
        * **per-reset** — ``pad_groups``, ``net_names``, ``routable_nets``:
        ...
        if reset_given:
            # Ladder weights (log OR linear shape) and/or the pad-group-scaled
            # board bonuses. Both read the SAME bare-board pad groups, so they
            # share one validation pass.
            want_weights = (
                self.net_bonus_size_log_scale > 0
                or self.net_bonus_size_linear_scale > 0
            )
            want_board_pg = (
                self.completion_bonus_padgroup_scale is not None
                or self.clean_completion_bonus_padgroup_scale is not None
            )
            if want_weights or want_board_pg:
                nets = sorted(int(c) for c in routable_nets)
                missing = [c for c in nets if c not in pad_groups]
                if missing:
                    raise RuntimeError(
                        f"pad-group-scaled ladder: routable net(s) {missing} "
                        f"have no pad groups — cannot resolve (stale "
                        f"connectivity? build_connectivity() must precede the "
                        f"bind)."
                    )
                if want_board_pg:
                    total_pg = sum(pad_groups[c] for c in nets)
                    if self.completion_bonus_padgroup_scale is not None:
                        self.completion_bonus = (
                            self.completion_bonus_padgroup_scale * total_pg
                        )
                    if self.clean_completion_bonus_padgroup_scale is not None:
                        self.clean_completion_bonus = (
                            self.clean_completion_bonus_padgroup_scale * total_pg
                        )
                if want_weights:
                    unnamed = [c for c in nets if c not in net_names]
                    if unnamed:
                        raise RuntimeError(
                            f"pad-group-scaled ladder: routable net(s) "
                            f"{unnamed} have no net name — cannot key the "
                            f"DRC-side weights."
                        )
                    if self.net_bonus_size_linear_scale > 0:
                        scale = self.net_bonus_size_linear_scale
                        by_code = {c: scale * pad_groups[c] for c in nets}
                    else:
                        scale = self.net_bonus_size_log_scale
                        by_code = {
                            c: scale * math.log1p(pad_groups[c]) for c in nets
                        }
